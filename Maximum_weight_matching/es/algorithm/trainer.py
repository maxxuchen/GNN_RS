...
class Trainer:
    ...
    def __init__(self, Policy, policy_path, instances, seed=1, num_workers=10, step_size=0.01, count=100,
        ...
        # currently, we use the fixed 100 instances for es
        # question: what about we choose many instances first, and randomly choose 100 among them
    ...
    def step(self, theta_id, min_evaluations, noise_std):
        ...
        while num_episodes < min_evaluations:
            ...
            for worker_seed in self.worker_seeds:
                ...
                worker = Worker.remote(seed=worker_seed, noise=self.noise_id, Solution=self.Policy,
                                       subworkers=subworker, min_task_runtime=self.min_task_runtime)
        ...
        wandb.log({"cross_entropy": np.mean(score)})
